          ...
          <div style={{ display: 'flex', width: '100%', height: '100%', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, height: '100%', overflow: 'auto', padding: 24, background: 'var(--bg)' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, gap: 16, flexWrap: 'wrap' }}>
                ...
                {/* Roscos: % de cultivo por finca (por superficie). Arriba a la derecha; si hay muchas fincas, forman varias filas. */}
                {roscosAMostrar.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-end', gap: 14, flex: 1, minWidth: 0 }}>
                    {roscosAMostrar.map(({ finca, data }) => (
                      <div key={finca.id} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '8px 12px' }}>
                        <div style={{ position: 'relative', width: 72, height: 72, flexShrink: 0 }}>
                          <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                              <Pie data={data} dataKey="sup" nameKey="cultivo" innerRadius={20} outerRadius={34} startAngle={90} endAngle={-270} stroke="none">
                          ...
                          <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 4 }}>
                            <span style={{ fontSize: 8, fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--text)', lineHeight: 1.1, wordBreak: 'break-word' }}>{finca.nombre}</span>
                          </div>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                          {data.map((d, i) => (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                              <span style={{ width: 7, height: 7, borderRadius: '50%', background: colorDeCultivo(d.cultivo), flexShrink: 0 }} />
                              <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--text)', whiteSpace: 'nowrap' }}>{d.cultivo}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 400, padding: 24 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      ...
                      <button onClick={() => setFormFinca(false)} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                    </div>
                  </div>
                </div>
              )}
