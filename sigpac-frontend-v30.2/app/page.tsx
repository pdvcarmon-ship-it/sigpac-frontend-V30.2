...
export default function Home() {
  ...
  // Numeración dinámica de los pasos del asistente de tratamiento (cultivo → finca → personal → parcelas)
  ...
  const pasoNumPersonal = pasoPersonalVisible ? (pasoFincaVisible ? 3 : 2) : null
  const pasoNumParcelas = 1 + (pasoFincaVisible ? 1 : 0) + (pasoPersonalVisible ? 1 : 0) + 1
  ...
  const abrirFormTrat = (parcelaId?: string, trat?: Tratamiento, personalId?: string) => {
    ...
    setTCultivoSeleccionado(trat?.cultivoTrat || '')
    setTFincaSeleccionada(trat?.fincaId || '')
    setFitoBusqueda('')
    setFitoResultados([])
  ...
  const guardarTratamiento = async () => {
    ...
    const nuevo: Tratamiento = {
      ...
      fincaId: tFincaSeleccionada || undefined,
      cultivoTrat: tCultivoSeleccionado || undefined,
    }
  ...
  interface StatProducto {
    ...
    detalleParcelas: { nombre: string; dosisCanon: number; supHa: number; totalCanon: number; totalConsumidoCanon: number }[]
  }
  ...
  const buscarPorReferenciaSigpac = async () => {
    ...
    setSeleccionando(false); setMododibujo(false); setEstado('cargando_parcela'); setError('')
    setParcGeojson(null); setParcelaInfo(null); setProductos([])
    setParcelaSupHa(0); setParcelaVistaEnMapa(null); setFincaVistaEnMapa(null); resetear()
    setPanelAbierto(true)
    try {
          ...
          <div style={{ display: 'flex', width: '100%', height: '100%', position: 'relative' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '0 16px 16px' : '0', flexDirection: 'column', gap: 14, display: panelAbierto || !isMobile ? 'flex' : 'none' }}>
              ...
              <section>
                ...
                <button onClick={() => { const next = !seleccionando; setSeleccionando(next); setMododibujo(false); if (isMobile && next) setPanelAbierto(false) }} style={{ width: '100%', padding: '10px', borderRadius: 8, background: seleccionando ? 'var(--green)' : 'var(--surface2)', border: `1px solid ${seleccionando ? 'var(--green)' : 'var(--border)'}`, color: seleccionando ? 'var(--bg)' : 'var(--text)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s' }}>
              ...
              <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'flex-start', justifyContent: 'space-between', marginBottom: 20, gap: 16, flexWrap: isMobile ? 'nowrap' : 'wrap' }}>
                <div>
                  ...
                  <button onClick={() => { setFincaEditar(null); setFNombre(''); setFDescripcion(''); setFError(''); setFormFinca(true) }}
                    style={{ padding: '5px 12px', borderRadius: 6, background: 'rgba(61,220,110,0.1)', border: '1px solid rgba(61,220,110,0.3)', color: 'var(--green)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer', fontWeight: 700 }}>
                    + Crear finca
                  </button>
                </div>
            ...
            {/* Resumen total parcelas (respeta el filtro de finca activo) */}
              <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 20px', flex: 1 }}>
                  ...
                  <div style={{ fontSize: 24, fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--green)' }}>{(filtroFinca ? misParcelas.filter(p => p.fincaId === filtroFinca) : misParcelas).length}</div>
                </div>
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 20px', flex: 1 }}>
                  ...
                  <div style={{ fontSize: 24, fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--blue)' }}>{(filtroFinca ? misParcelas.filter(p => p.fincaId === filtroFinca) : misParcelas).reduce((a, p) => a + p.supHa, 0).toFixed(2)} ha</div>
                </div>
              </div>
              ...
              {(filtroFinca ? misParcelas.filter(p => p.fincaId === filtroFinca) : misParcelas).length === 0 ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(260px, 1fr))', gap: isMobile ? 10 : 16 }}>
                  {(filtroFinca ? misParcelas.filter(p => p.fincaId === filtroFinca) : misParcelas).map(p => (
                    <div
                      key={p.id}
                    ...
                    >
                      ...
                      <div style={{ padding: '12px 14px', position: 'relative' }}>
                        {p.fincaId && fincas.find(f => f.id === p.fincaId) && (
                          <div style={{ position: 'absolute', top: 12, right: 14, maxWidth: '45%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)' }}>
              ...
              {!tratParcelaCtx && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <select value={tCultivoSeleccionado} onChange={e => { setTCultivoSeleccionado(e.target.value); setTFincaSeleccionada(''); setTParcelas([]); setTRecomendadoPor(''); setTAplicadoPor('') }}
                  ...
                  {tCultivoSeleccionado && pasoFincaVisible && (
                    <div>
                      ...
                      <select value={tFincaSeleccionada} onChange={e => { setTFincaSeleccionada(e.target.value); setTParcelas([]); setTRecomendadoPor(''); setTAplicadoPor('') }}
